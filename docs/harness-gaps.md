...
|-------|---------|----------|
| Pre-commit hooks | Local quality issues before push | ruff formatting, lint errors, type errors |
| Risk policy gate | Mis-classified PR risk | Tier 3 change merged without manual review |
| CI pipeline | Build/test/lint failures | Broken pytest, ruff violations, vitest failures |
| Code review | Logic errors, missing tests | Untested edge case, missing error handling |
| Browser evidence | Visual/UX regressions | Broken layout, missing UI element |
| Architectural linter | Boundary violations | Routes importing models directly, circular deps |
