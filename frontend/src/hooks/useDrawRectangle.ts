...
function ensureSources(map: maplibregl.Map) {
  /** add rectangle drawing sources and layers. */
...
function clearSources(map: maplibregl.Map) {
  /** clear all rectangle drawing source data. */
...
function removeSources(map: maplibregl.Map) {
  /** remove all rectangle drawing layers and sources. */
  for (const lyr of [LYR_LABELS, LYR_VERTICES, LYR_STROKE, LYR_FILL]) {
...
function rectRing(c1: [number, number], c2: [number, number]): [number, number][] {
  /** compute axis-aligned rectangle ring from two opposite corners. */
  return [
    c1,
    [c2[0], c1[1]],
    c2,
...
export default function useDrawRectangle(
  ...
  /** rectangle drawing tool - click two opposite corners. */
  ...
  useEffect(() => {
    ...
    function handleClick(e: maplibregl.MapMouseEvent) {
      ...
      if (!corner1Ref.current) {
        ...
        const ring = rectRing(corner1Ref.current, lngLat);
