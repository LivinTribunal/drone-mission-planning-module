...
def list_drones(current_user: User = Depends(require_operator), db: Session = Depends(get_db)):
...
def get_drone(
    drone_id: UUID, current_user: User = Depends(require_operator), db: Session = Depends(get_db)
):
