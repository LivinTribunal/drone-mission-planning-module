...
def list_missions(
    ...
    limit: int = 20,
    offset: int = 0,
    allowed_airport_ids: list[UUID] | None = None,
    ...
    if allowed_airport_ids is not None:
        query = query.filter(Mission.airport_id.in_(allowed_airport_ids))
    if airport_id:
    ...
    if allowed_airport_ids is not None:
        count_query = count_query.filter(Mission.airport_id.in_(allowed_airport_ids))
    if airport_id:
